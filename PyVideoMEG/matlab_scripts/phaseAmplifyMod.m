...
function res = phaseAmplify(vidFile, magPhase , fl, fh,fs, outDir, varargin)
    ...
    %%% Creates an anonymous function buildLevel(im_dft, k){ ifft2...}
    ...
    for k = 1:nF
        %%% Convert to YIQ colorspace - Luma, (orange-blue), (purple-green)
    ...
    for level = 2:numLevels-1
        ...
        %%% Can we drop this assumption???
        pyrRef = buildLevel(vidFFT(:,:,refFrame), level);        
        %%% Is the phase always calculated against refFrame?
        %%% Seems it is calculated later
        ...
        for frameIDX = 1:nF
            ...
            %%% The change of phase against the reference Frame.
